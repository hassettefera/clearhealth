...
// Medical Database - Rewritten for Plain English
...
function renderCard() {
  // Cancel speech synthesis if it's currently running when switching card contents
...
function speakExplanation() {
  ...
  const titleText = document.getElementById('res-title').innerText;
  const bodyText = document.getElementById('res-what-text').innerText;
  const fullTextToRead = `${titleText}. ${bodyText}`;

  if (!fullTextToRead.trim()) return;

  const utterance = new SpeechSynthesisUtterance(fullTextToRead);
  
  // Set voice language code to match language selection
  const langMap = {
    en: 'en-US',
    es: 'es-ES',
    zh: 'zh-CN',
  ...
  utterance.rate = 0.95; // Slightly calmer, clear speaking pace
